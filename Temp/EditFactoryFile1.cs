...
{
    public class EditFactoryFile1:CodeBuildService 
    {
      ...
      //      CodeFileReBuild(_iPath, sb2.ToString());
        ...
        private string EntityCode(EntityInfo entity)
        {
            ...
            foreach (FieldInfo field in entity .Fields )
            {
                if (field.FieldType == FieldType.ComplexField)
                {
                    ...
                    sbval.AppendInCode(4, "{0} = ({2}){1}", field.Name, field.Name.ToLower(),field .DataType  );
                }
                else if (!field.MainKey )
                {
                    paramList.Add(string.Format("{0} {1}", field.DataType, field.Name.ToLower()));
                    sbval.AppendInCode(4, "{0} = {1}", field.Name, field.Name.ToLower());
                }
            }
            sb.AppendLine();
            sb.AppendInCode(2, "public {0} Create{1}({2})", entity.IName, entity.Name,string .Join (",",paramList .ToArray ()));
            sb.AppendInCode(2, "{{");
        ...
        private string IEntityCode(EntityInfo entity)
        {
            ...
            return string.Format("        I{0} Create{0}({1});{2}", entity.Name, string.Join(",", paramList.ToArray()),Environment .NewLine );
        }
    }
}
